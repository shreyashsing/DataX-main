...
// Import the simplified token bytecode and ABI
import { SimplifiedDatatokenBytecode, SimplifiedDatatokenAbi } from '@/lib/contracts/bytecode/SimplifiedDatatoken';
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('Preparing transaction for deploying simplified token for NFT:', nftId);
    ...
    try {
      ...
      // Get the simplified DataToken contract bytecode and ABI
      console.log('Using simplified DataToken contract with reduced memory footprint');
      
      // Encode constructor parameters
      const tokenInterface = new ethers.utils.Interface(SimplifiedDatatokenAbi);
      const constructorData = tokenInterface.encodeDeploy([
        tokenName,
        tokenSymbol
      ]);
      
      // Create transaction to deploy simplified DataToken directly
      const deployData = SimplifiedDatatokenBytecode + constructorData.slice(2); // Remove 0x prefix from constructor data
      ...
      const unsignedTx = {
        from: walletAddress,
        gasLimit: ethers.utils.hexlify(5000000), // Lower gas limit for the simplified contract
      ...
      return NextResponse.json({
        ...
        nftId: nftId,
        message: 'Transaction prepared for simplified DataToken deployment',
        hint: "After deployment, use the /api/link-token API to link this token to your NFT"
      });
